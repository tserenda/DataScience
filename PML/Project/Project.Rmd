...
There are many variates that are the transformations of the true signals. The following are some of these variates: min, max, average, variance, standard deviation. By dropping these variates, we are not losing valuable information. Therefore, let's only pick the accelerometer measurements.
...
We'll use one of the most widely used classifier, Random Forest, with 10 fold of cross validations. The output is modeled with all the remaining covariates. Since several of the variants are highly skewed and/or have kurtosis (taller peak than normal distribution), we should use the Box Cox transformation. We should also run PCA to compress the features.
...
Also, as we can see the OOB error rate is 3%.
